...
FILES=$(find "$DATA_DIR" -maxdepth 1 -name "*.png" | sort)
if [ -z "$FILES" ]; then
    echo "Error: No .png files found in directory $DATA_DIR."
    exit 1
fi
...
if [ "$TOTAL_FILES" -lt "$K" ]; then
    echo "Error: Number of .png files ($TOTAL_FILES) is less than K ($K)."
    exit 1
fi
